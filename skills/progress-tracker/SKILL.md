...
1. Read `skills/spec-sync/specs/07-schedule.md` (Project Schedule)
...
1. Fix progress tracking in `GOAGENTX_DEV_SPEC.md`
...
**Option 1: Fix `GOAGENTX_DEV_SPEC.md`**
1. User provides corrected progress state
2. Update `GOAGENTX_DEV_SPEC.md` directly (the GLOBAL file)
3. Run `python skills/spec-sync/sync_spec.py`
...
Spec Reference:
  Schedule: specs/07-schedule.md (line XX)
...
| Task Name | `配置加载与校验` |
| Phase | `A - 基础架构搭建` |
| Spec Schedule Reference | `specs/07-schedule.md` line 142 |
...
6. **Graceful Degradation**: If spec files are missing, fall back to reading `GOAGENTX_DEV_SPEC.md` directly.
