---
name: spec-sync
description: Synchronize GOAGENTX_DEV_SPEC.md and split it into chapter-specific files under specs/ directory. Run sync_spec.py to update, then read SPEC_INDEX.md for navigation. Foundation for all spec-based operations. Use when user says "同步规范", "sync spec", or before any spec-dependent task.
metadata:
...
This skill synchronizes the master specification document (`GOAGENTX_DEV_SPEC.md`) and splits it into smaller, chapter-specific files stored in the `specs/` directory.
...
- You edited `GOAGENTX_DEV_SPEC.md` outside of workflow
...
python skills/spec-sync/sync_spec.py --force
```
...
1. Read `GOAGENTX_DEV_SPEC.md` from project root
...
```
Read: skills/spec-sync/SPEC_INDEX.md
```
...
```
Read: skills/spec-sync/specs/06-architecture.md
```
...
- **Always edit `GOAGENTX_DEV_SPEC.md`** and re-run the sync script
