...
stay in sync with the master GOAGENTX_DEV_SPEC.md file.
...
- **Hash-based Sync**: Only regenerates when GOAGENTX_DEV_SPEC.md actually changes
...
class Chapter(NamedTuple):
    ...
    start_line: int       # Start line in GOAGENTX_DEV_SPEC.md
    end_line: int         # End line in GOAGENTX_DEV_SPEC.md
    line_count: int       # Number of lines in this chapter


class SpecSynchronizer:
    """Synchronizes GOAGENTX_DEV_SPEC.md with decomposed chapter files"""
    ...
    def __init__(self, repo_root: Path):
        ...
        self.dev_spec_path = repo_root / "GOAGENTX_DEV_SPEC.md"
        self.skills_dir = repo_root / "skills" / "spec-sync"
    ...
    def calculate_hash(self) -> str:
        """Calculate SHA256 hash of GOAGENTX_DEV_SPEC.md"""
        if not self.dev_spec_path.exists():
            raise FileNotFoundError(f"GOAGENTX_DEV_SPEC.md not found at {self.dev_spec_path}")
    ...
    def needs_sync(self, force: bool = False) -> bool:
        ...
        if current_hash != stored_hash:
            print("🔄 GOAGENTX_DEV_SPEC.md has changed. Re-sync required.")
            return True
    ...
    def auto_detect_chapters(self, content: str) -> List[Chapter]:
        ...
        if not chapter_starts:
            raise ValueError("No chapters found in GOAGENTX_DEV_SPEC.md. Expected format: '## N. Title'")
    ...
    def split_spec(self):
        """Split GOAGENTX_DEV_SPEC.md into chapter files"""
        print("📄 Reading GOAGENTX_DEV_SPEC.md...")
...
> **Purpose**: This index helps you quickly locate the relevant chapter without reading the entire GOAGENTX_DEV_SPEC.md.
> **Auto-Generated**: This file is automatically generated by `sync_spec.py`. Do not edit manually.
> **Last Sync**: Based on GOAGENTX_DEV_SPEC.md with {len(self.chapters)} chapters detected.
...
2. **Always edit `GOAGENTX_DEV_SPEC.md`** at the repo root, then run `sync_spec.py`
3. **Adding new chapters**: Just add `## N. Title` in GOAGENTX_DEV_SPEC.md and update `TITLE_SLUG_MAP` in sync_spec.py
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Sync GOAGENTX_DEV_SPEC.md with Skills directory")
    parser.add_argument('--force', action='store_true', help="Force regeneration even if hash matches")
    args = parser.parse_args()
    
    # Detect repo root (script is in skills/spec-sync/)
