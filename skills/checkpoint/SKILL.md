---
name: checkpoint
description: Summarize completed work, update progress tracking in GOAGENTX_DEV_SPEC.md, and prepare for next iteration. Final stage of dev-workflow pipeline. Use when task implementation and testing is completed, or when user says "完成检查点", "checkpoint", "保存进度", "save progress", "任务完成".
metadata:
...
This skill handles **task completion summarization** and **progress tracking synchronization**. It ensures that completed work is properly documented and the project schedule in `GOAGENTX_DEV_SPEC.md` stays up-to-date.
...
- When you need to **manually update progress** in `GOAGENTX_DEV_SPEC.md`
...
────────────────────────────────────────────────────
 ...
 Spec Reference: GOAGENTX_DEV_SPEC.md Section [X.Y]
────────────────────────────────────────────────────
```
...
════════════════════════════════════════════════════
 ...
 以上总结是否准确？
   ...
   Please reply: "confirm" / "确认" to save progress to GOAGENTX_DEV_SPEC.md
                "revise" / "修改" to regenerate summary
                
 Note: This only verifies the summary. GOAGENTX_DEV_SPEC.md will be updated
...
**Goal**: Update `GOAGENTX_DEV_SPEC.md` to mark the task as completed.

> **Auto-Execute**: This step runs automatically after Step 1.5 user confirmation. No additional user input required.

### 2.1 Locate Task in GOAGENTX_DEV_SPEC.md

1. Read `GOAGENTX_DEV_SPEC.md` (the **GLOBAL** file, NOT chapter files)
...
**Output after updating GOAGENTX_DEV_SPEC.md**:
```
────────────────────────────────────
GOAGENTX_DEV_SPEC.md Progress Updated
────────────────────────────────────
Task: [Task ID] [Task Name]
Status: [ ] -> [x]
────────────────────────────────────
...
【Description】
Completed GOAGENTX_DEV_SPEC.md Phase X.Y: <Task Name>
...
Refs: GOAGENTX_DEV_SPEC.md Section X.Y
Task: [Task ID] <Task Name>
...
1. **Always Update GLOBAL GOAGENTX_DEV_SPEC.md**: This is the single source of truth for progress tracking.
